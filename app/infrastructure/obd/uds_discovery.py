...
from app.infrastructure.persistence.vin_cache import get_vin_cache, set_vin_cache
from app.domain.ports import ScannerPort, UdsDiscoveryPort
...
class UdsDiscoveryService(UdsDiscoveryPort):
    def discover(self, scanner: ScannerPort, options: Dict[str, Any]) -> Dict[str, Any]:
        elm = scanner.get_transport()
        opts = DiscoveryOptions(**options) if options else DiscoveryOptions()
        return discover_uds_modules(elm, opts)
