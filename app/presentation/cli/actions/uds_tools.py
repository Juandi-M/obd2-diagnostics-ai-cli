...
def _select_module(state: AppState, brand: str) -> Optional[Dict[str, Any]]:
    modules: Dict[str, Dict[str, str]] = get_container().uds_tools.module_map(brand)
...
def uds_tools_menu(state: AppState) -> None:
    ...
    try:
        client = get_container().uds_tools.build_client(brand, module_entry)
...
def _read_vin(client, brand: str) -> None:
    try:
...
def _read_did(client, brand: str) -> None:
...
def _discover_modules(state: AppState) -> None:
    ...
    try:
        result = get_container().uds_discovery.discover(options)
...
def _cached_map_for_state(state: AppState) -> Optional[Dict[str, Any]]:
    ...
    if not vin:
        return None
    cached = get_container().vin_cache.get(vin) or {}
