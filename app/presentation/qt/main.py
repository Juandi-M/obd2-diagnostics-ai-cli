...
from PySide6.QtCore import QObject, QThreadPool, Qt, Signal, QTimer, QPropertyAnimation
...
class ConnectPage(QWidget):
    ...
    def _scan(self) -> None:
        ...
        if self.usb_radio.isChecked():
            self.connection_vm.scan_usb()
            return

        include_all = self.show_all_ble.isChecked()
        self.connection_vm.scan_ble(include_all)

    def _on_usb_scan(self, result: Any, err: Any) -> None:
        if err:
            QMessageBox.warning(self, "USB", f"Scan error: {err}")
            return
        ports = result or []
        for port in ports:
    ...
    def _connect(self) -> None:
        ...
        self.connection_vm.connect_device(port, use_kline)
