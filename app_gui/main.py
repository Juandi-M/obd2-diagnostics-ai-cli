...
from PySide6.QtCore import QObject, QRunnable, QThreadPool, Qt, Signal, QTimer, QPropertyAnimation
...
from app_core.ai_report import decode_vin_with_ai, decode_vin_with_vpic, request_ai_report
from app_core.connection import scan_ble_devices, scan_usb_ports, try_connect
...
from obd.dtc.database import DTCDatabase
...
from paywall.config import get_api_base, is_bypass_enabled, load_balance, pending_total
...
GUI_I18N = {
    "en": {
        ...
        "legacy_kline": "Legacy K-Line fallback (USB)",
    ...
    "es": {
        ...
        "legacy_kline": "Fallback K-Line (USB)",
...
class StatusLabel(QLabel):
    ...
    def update_text(self) -> None:
        ...
        protocol = "K-LINE" if self.state.legacy_scanner and self.state.legacy_scanner.is_connected else "OBD2"
...
class Gauge(QWidget):
    ...
    def paintEvent(self, event) -> None:
        ...
        try:
            ...
            pen_val = QPen(QColor("#6d6cff"), 8)
            painter.setPen(pen_val)
            painter.drawArc(arc_rect, 225 * 16, int(270 * 16 * pct))
        finally:
            painter.end()
...
class ChartPanel(QWidget):
    ...
    def paintEvent(self, event) -> None:
        ...
        try:
            ...
            painter.setPen(QPen(QColor("#d9d7f5"), 1))
            painter.drawRoundedRect(rect, 8, 8)

            painter.setPen(QPen(QColor("#8a8fb8"), 1))
            ...
            painter.setPen(QPen(QColor("#e9e7fb"), 1, Qt.DashLine))
            for i in range(1, 4):
                y = chart_rect.top() + i * (chart_rect.height() / 4)
                painter.drawLine(chart_rect.left(), y, chart_rect.right(), y)

            painter.setPen(QPen(QColor("#6d6cff"), 2))
            prev = None
            for i, v in enumerate(self.values):
...
def _try_kline_gui(state: AppState, port: str) -> Tuple[bool, Dict[str, Any], Optional[Exception]]:
    try:
    ...
    try:
        ...
        info = {
            "protocol": "K-LINE",
            "profile": session.info.profile_name,
            "reason": session.info.reason,
        }
        return True, info, None
    except (KLineDetectError, KLineError) as exc:
        try:
            elm.close()
        except Exception:
            pass
...
def export_text_pdf(lines: List[str], output_path: Any) -> None:
    try:
...
class SetupPage(QWidget):
    def __init__(self, state: AppState, on_continue: Callable[[], None]) -> None:
        ...
        for opt_id, label, _, _, _ in get_brand_options():
        ...
        layout.addWidget(panel, alignment=Qt.AlignHCenter)
    ...
    def _brand_changed(self) -> None:
        ...
        if brand_id is None:
            return
        apply_brand_selection(self.state, str(brand_id))
        self._load_from_state()
        self._apply_brand_lock()
    ...
    def _save(self) -> None:
        ...
        save_settings(settings_from_state(self.state))
        self.on_continue()
...
class ConnectPage(QWidget):
    def __init__(
        ...
        self.kline_checkbox = QCheckBox(gui_t(self.state, "legacy_kline"))
        ...
        panel_layout_.addWidget(connect_btn)
        layout.addWidget(panel, alignment=Qt.AlignHCenter)
        ...
        apply_shadow(self.empty_card, blur=14, y=4)
        self.empty_card.setMaximumWidth(720)
        empty_layout = QVBoxLayout(self.empty_card)
        self.empty_title = QLabel(gui_t(self.state, "connect_empty_title"))
        self.empty_title.setObjectName("sectionTitle")
        self.empty_text = QLabel(gui_t(self.state, "connect_empty_body"))
        self.empty_text.setWordWrap(True)
        empty_layout.addWidget(self.empty_title)
        empty_layout.addWidget(self.empty_text)
        layout.addWidget(self.empty_card, alignment=Qt.AlignHCenter)
    ...
    def refresh_text(self) -> None:
        ...
        self.kline_checkbox.setText(gui_t(self.state, "legacy_kline"))
        ...
        self.empty_title.setText(gui_t(self.state, "connect_empty_title"))
        self.empty_text.setText(gui_t(self.state, "connect_empty_body"))
    ...
    def update_empty_state(self) -> None:
        self.empty_card.setVisible(self.state.active_scanner() is None)
    ...
    def _scan(self) -> None:
        ...
        if self.usb_radio.isChecked():
            ...
            if not ports:
                QMessageBox.information(self, "USB", "No USB adapters detected.")
            return

        include_all = self.show_all_ble.isChecked()
        worker = Worker(scan_ble_devices, include_all)
        worker.signals.finished.connect(self._on_ble_scan)
        self.thread_pool.start(worker)

    def _on_ble_scan(self, result: Any, err: Any) -> None:
        if err:
            QMessageBox.warning(self, "Bluetooth", f"Scan error: {err}")
            return
        devices, ble_err = result
        if ble_err:
            QMessageBox.warning(self, "Bluetooth", str(ble_err))
        ...
        if not self.device_list:
            QMessageBox.information(self, "Bluetooth", "No BLE adapters detected.")

    def _populate_list(self) -> None:
        self.list_widget.clear()
        for port, name, rssi in self.device_list:
            rssi_label = f" | {rssi} dBm" if isinstance(rssi, int) and rssi > -999 else ""
            item = QListWidgetItem(f"{name} [{port}]{rssi_label}")
    ...
    def _connect(self) -> None:
        ...
        worker.signals.finished.connect(self._on_connect)
        self.thread_pool.start(worker)

    def _on_connect(self, result: Any, err: Any) -> None:
        if err:
            QMessageBox.warning(self, "Connect", f"Connection error: {err}")
            return
        mode = result.get("mode") if isinstance(result, dict) else None
        info = result.get("info") if isinstance(result, dict) else None
        exc = result.get("error") if isinstance(result, dict) else None
        if mode not in {"obd", "kline"}:
            QMessageBox.warning(self, "Connect", f"Failed: {exc}")
            return
    ...
    def _connect_job(self, port: str, use_kline: bool) -> Dict[str, Any]:
        ...
        ok, info, exc = try_connect(scanner, port, self.state.raw_logger())
        if ok:
            return {"mode": "obd", "info": info, "error": None}
        if use_kline and not str(port).lower().startswith("ble:"):
            kline_ok, kline_info, kline_err = _try_kline_gui(self.state, port)
            if kline_ok:
                return {"mode": "kline", "info": kline_info, "error": None}
...
class MainMenuPage(QWidget):
    def __init__(self, state: AppState, on_select: Callable[[str], None], on_reconnect: Callable[[], None]) -> None:
        ...
        connect_panel.setMaximumWidth(760)
        ...
        layout.addWidget(connect_panel, alignment=Qt.AlignHCenter)
        ...
        self.tiles = [
            ("diagnose", "#c7d2fe", "diagnose"),
            ("live", "#d1fae5", "live"),
            ("ai_report", "#e9d5ff", "ai"),
            ("reports", "#bfdbfe", "reports"),
            ("settings", "#fde68a", "settings"),
            ("uds_tools", "#fbcfe8", "uds"),
            ("module_map", "#ddd6fe", "module_map"),
        ]
        positions = [
            (0, 0, 1, 1),
            (0, 1, 1, 1),
        ...
        for idx, (label_key, color, key) in enumerate(self.tiles):
            row, col, row_span, col_span = positions[idx]
            icon = self.tile_icons.get(key, "")
            btn = QPushButton(f"{icon} {gui_t(self.state, label_key)}".strip())
        ...
        tiles_panel.setMaximumWidth(760)
        tiles_layout.addLayout(grid)
        layout.addWidget(tiles_panel, alignment=Qt.AlignHCenter)
        layout.addStretch(1)
    ...
    def refresh_text(self) -> None:
        ...
        for btn, (label_key, _, key) in zip(self.tile_buttons, self.tiles):
            icon = self.tile_icons.get(key, "")
            btn.setText(f"{icon} {gui_t(self.state, label_key)}".strip())
...
class PaywallDialog(QDialog):
    ...
    def _refresh(self) -> None:
        ...
        subject = _short_id(self.client.identity.subject_id)
        ...
        try:
            ...
            self.pending_label.setText(f"Pending: {pending_total()}")
    ...
    def _reset_identity(self) -> None:
        from paywall.config import reset_identity
...
class DiagnosePage(QWidget):
    def __init__(
        ...
        self.quick_scan_btn = QPushButton(gui_t(self.state, "run_full_scan"))
        self.quick_scan_btn.setObjectName("chip")
        self.quick_scan_btn.clicked.connect(self._run_full_scan)
        self.quick_clear_btn = QPushButton(gui_t(self.state, "clear_codes_action"))
        self.quick_clear_btn.setObjectName("chip")
        self.quick_clear_btn.clicked.connect(self._run_clear_codes)
        quick_row.addWidget(quick_label)
        quick_row.addWidget(self.quick_scan_btn)
        ...
        self.lookup_input.setMaximumWidth(200)
        ...
        quick_row.addWidget(self.lookup_label)
        quick_row.addLayout(input_col)
        quick_row.addWidget(self.lookup_btn)
        quick_row.addWidget(self.lookup_clear_btn)
        quick_row.addStretch(1)
        panel_layout_.addLayout(quick_row)
        ...
        panel_layout_.addLayout(button_row)
    ...
    def refresh_text(self) -> None:
        ...
        self.quick_scan_btn.setText(gui_t(self.state, "run_full_scan"))
    ...
    def _set_busy(self, busy: bool) -> None:
        ...
        for btn in (self.quick_scan_btn, self.quick_clear_btn):
            btn.setEnabled(not busy)
    ...
    def _lookup_code(self) -> None:
        ...
        if not re.match(r"^[PBUC][0-3][0-9A-F]{3}$", code):
            ...
            self.lookup_result_body.setText(gui_t(self.state, "code_hint"))
            return
        dtc_db = DTCDatabase(manufacturer=self.state.manufacturer if self.state.manufacturer != "generic" else None)
    ...
    def _search_codes(self) -> None:
        ...
        if not query:
            ...
            return
        dtc_db = DTCDatabase(
            manufacturer=self.state.manufacturer if self.state.manufacturer != "generic" else None
        )
    ...
    def _job_freeze_frame(self) -> str:
        ...
        if not scanner:
            raise NotConnectedError("Not connected")
        freeze = read_freeze_frame(scanner)
        lines = []
...
class LiveDataPage(QWidget):
    def __init__(self, state: AppState, on_back: Callable[[], None], on_reconnect: Callable[[], None]) -> None:
        ...
        layout.addWidget(controls_panel, alignment=Qt.AlignHCenter)
        ...
        cards_panel.setMaximumWidth(980)
        cards_layout.addWidget(self.section_label)
        cards_layout.addLayout(self.grid)
        layout.addWidget(cards_panel, alignment=Qt.AlignHCenter)
        ...
        chart_row.addWidget(self.rpm_chart)
        chart_row.addWidget(self.speed_chart)
        chart_row.addWidget(self.throttle_chart)

        charts_panel, charts_layout = panel_layout(padding=16)
        charts_panel.setMaximumWidth(980)
        ...
        layout.addWidget(charts_panel, alignment=Qt.AlignHCenter)
    ...
    def _poll_job(self) -> Dict[str, Any]:
        ...
        return read_live_data(scanner, pids)
...
class AIReportPage(QWidget):
    def __init__(self, state: AppState, on_back: Callable[[], None], on_reconnect: Callable[[], None]) -> None:
        ...
        layout.addWidget(top_panel, alignment=Qt.AlignHCenter)
    ...
    def _refresh_balance(self) -> None:
        ...
        try:
            ...
            self.credits_label.setText(
                f"Credits: {balance.free_remaining} free / {balance.paid_credits} paid (pending {pending_total()})"
            )
        except PaywallError as exc:
    ...
    def _refresh_reports(self) -> None:
        ...
        for report in list_reports():
            ...
            if query and query not in report.report_id.lower():
                ...
                try:
                    payload = load_report(report.file_path)
                except Exception:
                    payload = {}
    ...
    def _load_selected_report(self) -> None:
        ...
        pdf_path = payload.get("pdf_path") or report_pdf_path(payload.get("report_id", report_id))
    ...
    def _generate_job(self, notes: str, use_vin_decode: bool) -> Dict[str, Any]:
        ...
        try:
            ...
            report_snapshot = load_report(report_path)
            ...
            if not is_bypass_enabled():
                client = PaywallClient()
                if not client.is_configured:
                    raise PaywallError("Paywall API base not configured")
    ...
    def _on_generate_done(self, result: Any, err: Any) -> None:
        ...
        self.loading_bar.setVisible(False)
        if err:
            if isinstance(err, PaymentRequired):
    ...
    def _prepare_vehicle_payload(
        ...
        if vin and use_vin_decode:
            ...
            if cached:
                ...
                vin_profile = decode_vin_with_vpic(vin, model_year=model_year)
                if vin_profile:
                    set_vin_cache(vin, vin_profile)
                if not vin_profile:
                    try:
                        vin_profile = decode_vin_with_ai(vin, self.state.manufacturer)
                    except OpenAIError:
                        vin_profile = None
                    if vin_profile:
                        set_vin_cache(vin, vin_profile)
...
class UdsToolsPage(QWidget):
    def __init__(self, state: AppState, on_back: Callable[[], None], on_reconnect: Callable[[], None]) -> None:
        ...
        action_row.addWidget(self.read_vin_btn)
        action_row.addWidget(self.read_did_btn)
        action_row.addWidget(self.read_dtcs_btn)
        action_row.addWidget(self.send_raw_btn)
        ...
        self.service_input.setMaximumWidth(120)
        self.data_input = QLineEdit()
        self.data_input.setPlaceholderText(gui_t(self.state, "uds_data_hex"))
        inputs_row.addWidget(QLabel(gui_t(self.state, "uds_read_did")))
        inputs_row.addWidget(self.did_input)
        inputs_row.addSpacing(16)
        inputs_row.addWidget(QLabel(gui_t(self.state, "uds_service_id")))
        inputs_row.addWidget(self.service_input)
        inputs_row.addWidget(QLabel(gui_t(self.state, "uds_data_hex")))
        inputs_row.addWidget(self.data_input)
        ...
        discovery_row.addWidget(self.discover_btn)
        discovery_row.addSpacing(8)
        discovery_row.addWidget(self.discover_quick)
        discovery_row.addWidget(self.discover_29)
        discovery_row.addWidget(self.discover_250)
        discovery_row.addWidget(self.discover_dtcs)
        discovery_row.addStretch(1)
        discovery_row.addWidget(timeout_label)
        discovery_row.addWidget(self.discover_timeout)
    ...
    def _refresh_cached_map(self) -> None:
        ...
        cached = get_vin_cache(vin) if vin else None
    ...
    def _ensure_connected(self) -> bool:
        if not self.state.active_scanner():
            ...
            return False
        if self.state.legacy_scanner and self.state.legacy_scanner.is_connected:
    ...
    def _create_client(self, brand: str, module_entry: Any) -> UdsClient:
        scanner = self.state.active_scanner()
        if not scanner:
            raise UdsResponseError("No vehicle connected.")
        if isinstance(module_entry, dict) and module_entry.get("tx_id"):
            return UdsClient(
                scanner.elm,
        ...
        return UdsClient.from_module(scanner.elm, brand, str(module_entry))
...
class ModuleMapPage(QWidget):
    def __init__(self, state: AppState, on_back: Callable[[], None], on_reconnect: Callable[[], None]) -> None:
        ...
        discovery_row.addWidget(self.discover_dtcs)
        discovery_row.addStretch(1)
        discovery_row.addWidget(QLabel(gui_t(self.state, "uds_discover_timeout")))
        discovery_row.addWidget(self.discover_timeout)
        panel_layout_.addLayout(discovery_row)
    ...
    def _run_discovery(self) -> None:
        ...
        def job() -> Dict[str, Any]:
            ...
            return discover_uds_modules(scanner.elm, options)
...
class AIReportViewer(QDialog):
    def __init__(self, report_path: Any, parent: Optional[QWidget] = None) -> None:
        ...
        self.payload = load_report(report_path)
...
class SettingsPage(QWidget):
    ...
    def _save(self) -> None:
        ...
        if isinstance(brand_id, str):
            apply_brand_selection(self.state, brand_id)
        self.state.vehicle_profile = {
            "make": self.make.text().strip() or None,
            "model": self.model.text().strip() or None,
            "year": self.year.text().strip() or None,
...
class MainWindow(QMainWindow):
    def __init__(self) -> None:
        ...
        self.state = AppState()
        apply_settings(self.state, load_settings())
